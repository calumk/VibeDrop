...
// .wrangler/tmp/bundle-mLl96V/checked-fetch.js
...
// .wrangler/tmp/bundle-mLl96V/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/bundle-mLl96V/middleware-loader.entry.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();

// .wrangler/tmp/bundle-mLl96V/middleware-insertion-facade.js
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
...
var Context = /* @__PURE__ */ __name(class {
  constructor(req, options) {
    ...
    this.streamText = (cb2, arg, headers) => {
      headers ?? (headers = {});
      this.header("content-type", TEXT_PLAIN);
      this.header("x-content-type-options", "nosniff");
      this.header("transfer-encoding", "chunked");
      return this.stream(cb2, arg, headers);
    };
    this.stream = (cb2, arg, headers) => {
      const { readable, writable } = new TransformStream();
      const stream = new StreamingApi(writable, readable);
      cb2(stream).finally(() => stream.close());
...
__name(initS3Client, "initS3Client");
async function createMultipartUpload(fileName, fileType) {
...
async function createMetadata(metadata) {
  ...
  const expiryDate = fileData.expiryDays ? new Date(now.getTime() + fileData.expiryDays * 24 * 60 * 60 * 1e3) : null;
  ...
  const command = new PutObjectCommand({
    ...
    Body: JSON.stringify({
      ...
      id: fileId,
      ...fileData,
      createdAt: existingMetadata.createdAt || now.toISOString(),
      // Keep original createdAt if it exists
      expiryDate: expiryDate ? expiryDate.toISOString() : null,
      downloadCount: existingMetadata.downloadCount || 0
...
__name(createMetadata, "createMetadata");
...
async function getDownloadUrl(fileId, passcode) {
...
async function listFiles() {
  ...
  try {
    ...
    return {
      success: true,
      files: files.sort((a2, b2) => b2.lastModified - a2.lastModified)
    };
  } catch (error) {
...
async function getUploadUrl(fileId, fileName, fileType) {
  const key = `files/${fileId}`;
  const command = new PutObjectCommand({
    Bucket: env.R2_BUCKET_NAME,
    Key: key,
...
__name(getUploadUrl, "getUploadUrl");

// routes/update-metadata.js
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
async function onRequest(c2) {
  try {
    const { fileId, metadata } = await c2.req.json();
    if (!fileId || !metadata) {
      return c2.json({
        success: false,
...
app.post("/api/create-multipart", async (c2) => {
  try {
    const { fileId, fileName, fileType } = await c2.req.json();
...
app.post("/api/sign-part", async (c2) => {
  try {
    const { key, uploadId, partNumber } = await c2.req.json();
...
app.post("/api/complete-multipart", async (c2) => {
  try {
    const { key, uploadId, parts } = await c2.req.json();
...
app.post("/api/create-metadata", async (c2) => {
  try {
    const metadata = await c2.req.json();
    if (!metadata) {
      return c2.json({ success: false, error: "Missing metadata" }, 400);
    }
...
app.post("/api/get-metadata", async (c2) => {
  try {
    const { fileId } = await c2.req.json();
    if (!fileId) {
      return c2.json({ success: false, error: "Missing fileId" }, 400);
    }
...
app.post("/api/get-download-url", async (c2) => {
  try {
    const { fileId, passcode } = await c2.req.json();
    if (!fileId) {
      return c2.json({ success: false, error: "Missing fileId" }, 400);
    }
...
app.get("/api/list-files", async (c2) => {
  try {
...
app.delete("/api/delete-file", async (c2) => {
  try {
    const fileId = c2.req.query("fileId");
    if (!fileId) {
      return c2.json({ success: false, error: "Missing fileId" }, 400);
    }
...
app.post("/api/clean-expired", async (c2) => {
  try {
...
app.post("/api/get-upload-url", async (c2) => {
  try {
    const { fileId, fileName, fileType } = await c2.req.json();
    if (!fileId || !fileName || !fileType) {
      return c2.json({ success: false, error: "Missing required fields" }, 400);
...
app.post("/api/update-metadata", onRequest);
